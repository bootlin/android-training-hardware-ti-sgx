...
# Auto-generated for omap4430_android from build: sgxddk_MAIN@956079

# PVR Consumer services version number
#
PVRVERSION="sgxddk_MAIN@956079"
...
install_pvr()
{
	$DOIT echo "version sgxddk_MAIN@956079" >${DISCIMAGE}${DDK_INSTALL_LOG}
...
if [ -z "$NO_DISPLAYMOD" ]; then
	...
	#
        install_file libGLESv1_CM_POWERVR_SGX530_125.so /system/lib/egl/libGLESv1_CM_POWERVR_SGX530_125.so "shared library" 0644 0:0
        


        install_file libusc.so /system/lib/libusc.so "shared library" 0644 0:0
        
	
        install_file libGLESv2_POWERVR_SGX530_125.so /system/lib/egl/libGLESv2_POWERVR_SGX530_125.so "shared library" 0644 0:0
        

        install_file libglslcompiler.so /system/lib/libglslcompiler.so "shared library" 0644 0:0
        
	


        install_file libIMGegl.so /system/lib/libIMGegl.so "shared library" 0644 0:0
        
	        install_file libEGL_POWERVR_SGX530_125.so /system/lib/egl/libEGL_POWERVR_SGX530_125.so "shared library" 0644 0:0
        
        install_file libpvr2d.so /system/lib/libpvr2d.so "shared library" 0644 0:0
        
	


        install_file libsrv_um.so /system/lib/libsrv_um.so "shared library" 0644 0:0
        
	        install_file libsrv_init.so /system/lib/libsrv_init.so "shared library" 0644 0:0
        
	        install_file libPVRScopeServices.so /system/lib/libPVRScopeServices.so "shared library" 0644 0:0
...
echo
echo "Installing PowerVR Consumer/Embedded DDK 'sgxddk_MAIN@956079' on $from"
echo
