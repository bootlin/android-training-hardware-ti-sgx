...
static PVRSRV_ERROR AcquireGPTimer(SYS_SPECIFIC_DATA *psSysSpecData)
{
	...
	psSysSpecData->psGPT7_FCK = psCLK;
	...
	psSysSpecData->psGPT7_ICK = psCLK;
	...
	if(clk_get_parent(psSysSpecData->psGPT7_FCK) != sys_ck)
	{
		PVR_TRACE(("Setting GPTIMER parent to System Clock"));
		res = clk_set_parent(psSysSpecData->psGPT7_FCK, sys_ck);
		if (res < 0)
		{
	...
	rate = clk_get_rate(psSysSpecData->psGPT7_FCK);
	PVR_TRACE(("GPTIMER clock is %dMHz", HZ_TO_MHZ(rate)));

	res = clk_enable(psSysSpecData->psGPT7_FCK);
	if (res < 0)
	{
...
ExitDisableGPT7ICK:
#if defined(PVR_OMAP4_TIMING_PRCM)
	clk_disable(psSysSpecData->psGPT7_ICK);
ExitDisableGPT7FCK:
	clk_disable(psSysSpecData->psGPT7_FCK);
ExitError:
#endif	
